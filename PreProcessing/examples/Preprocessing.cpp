...
int main(int argc, char ** argv) {
    if (argc<4)
    {
        fprintf(stderr, "usage: pagerank [path] [cache size in MB] [graph size in MB] [memory budget in MB]\n");
        exit(-1);
    }
    std::string path = argv[1];
    long cache_size = atol(argv[2])*1024l*1024l;
    long graph_size = atol(argv[3])*1024l*1024l;
    long memory_bytes = (argc>=5)?atol(argv[4])*1024l*1024l:8l*1024l*1024l*1024l;
